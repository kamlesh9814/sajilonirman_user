...
class ChatProvider extends ChangeNotifier {
  ...
  bool adminmessagesendLoading = false;
  ...
  Future<void> getChatList(BuildContext context, int offset,
    ...
    if (apiResponse.response != null &&
      ...
      if (offset == 1) {
        ...
        log("chatdetails");
        log(chatModel!.chat!.length.toString());
      } else {
        chatModel?.chat
  ...
  List<AdminChatViewModel> adminchatlists = [];
  Future<void> getAdminChatList(int userId) async {
    try {
      ApiResponse apiresponse = await chatRepo!.getAdminChatList(userId);
      if (apiresponse.response!.data['datas'] != null &&
          apiresponse.response!.statusCode == 200) {
        adminchatlists.clear();
        apiresponse.response!.data['datas'].forEach((adminchatlist) =>
            adminchatlists.add(AdminChatViewModel.fromJson(adminchatlist)));
        print("ChatwithAdmin");
        print(adminchatlists);
      } else {
        print("error in adminlistelse");
      }
    } catch (e) {
      print("errorwithchatis.............");
      print(e.toString());
    }
  }
  ...
  sendMessagetoAdmin(
      int? userId, String? message, int? adminId, BuildContext context) async {
    try {
      adminmessagesendLoading = true;
      notifyListeners();
      ApiResponse apiResponse =
          await chatRepo!.sendMessagetoAdmin(userId, message, adminId);
      if (apiResponse.response!.statusCode == 200) {
        showCustomSnackBarSuccess(
            apiResponse.response!.data['message'], context);
      } else {
        print("Problem in sending the message");
      }
    } catch (e) {
      print(e);
    } finally {
      adminmessagesendLoading = false;
      notifyListeners();
    }
  }
}
