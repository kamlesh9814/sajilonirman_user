...
class AuthProvider with ChangeNotifier {
  ...
  Future login(email, password, isemail, Function callback) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await authRepo!.login(email, password, isemail);
    _isLoading = false;
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      clearGuestId();
  ...
  Future<ResponseModel> forgetPassword(String email) async {
    print("phonenumberforforgotpassword" + email);
