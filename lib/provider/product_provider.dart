...
import 'package:flutter_sixvalley_ecommerce/data/model/response/adminproductmodel.dart';
...
//Admin product
  List<AdminProduct>? _adminProductList = [];
  // List<AdminProduct>? _adminProductList;
  List<AdminProduct>? get adminProductList => _adminProductList;
  ...
  Future<void> getadminProductList() async {
    ApiResponse apiResponse = await productRepo!.getadminProductList();
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _adminProductList = [];
      _adminProductList?.addAll(
          AdminProductDetailsModel.fromJson(apiResponse.response!.data)
              .products!);
      _adminProductList!.forEach((product) {
        print("name is${product.name}");
        print(product.addedBy);
      });
