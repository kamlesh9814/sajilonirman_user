...
class ChatRepo {
  ...
  Future<ApiResponse> getChatList(String type, int offset) async {
    ...
    Future<ApiResponse> getAdminChatList(int userId) async {
    try {
      final response = await dioClient!.get('${AppConstants.adminChatInfoUri}$userId');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> sendMessagetoAdmin(int? userId, String? message, int? adminId)async {
    try {
      final response = await dioClient!.post(AppConstants.baseUrl+
        AppConstants.messageSendtoAdmin,
        data: {
          "user_id":userId,
          "message":message,
          "admin_id":adminId
